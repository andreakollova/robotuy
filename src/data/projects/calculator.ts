      ...
      steps: [
        {
          ...
          prompt: 'Vstup od používateľa prichádza ako text "12.5". Aby sme s ním mohli počítať, musíme ho zmeniť na desatinné číslo. Doplň názov funkcie, ktorá to spraví.',
          fillCode: 'number = ___("12.5")',
          fillBlanks: [{ id: 'b1', answer: 'float', alternatives: ['Float'] }],
          hints: [
            { text: 'Potrebujeme funkciu, ktorá zmení text na desatinné číslo.' },
            { text: 'Desatinné čísla majú v Pythone typ float.' },
            { text: 'Použi názov tohto typu ako funkciu:', code: 'float(...)' },
          ],
          xp: 15,
        },
        {
          ...
          prompt: 'Kalkulačka sa musí rozhodnúť podľa zvolenej operácie. Podmienka if porovnáva hodnotu premennej operation s textom "+". Doplň operátor, ktorý porovnáva dve hodnoty (nie priraďuje!).',
          fillCode: 'if operation ___ "+":\n    result = first_number + second_number',
          fillBlanks: [{ id: 'b1', answer: '==' }],
          hints: [
            { text: 'Jeden znak = priraďuje hodnotu. Na porovnanie potrebujeme iný operátor.' },
            { text: 'Operátor porovnania používa dva znaky rovnosti.' },
            { text: 'Doplň:', code: '==' },
          ],
          xp: 15,
        },
        {
          ...
          prompt: 'Funkcia add() sčíta dve čísla a výsledok uloží do premennej result. Teraz musí tento výsledok odovzdať späť tam, odkiaľ bola zavolaná. Doplň príkaz, ktorý vráti hodnotu z funkcie.',
          fillCode: 'def add(first_number, second_number):\n    result = first_number + second_number\n    ___ result',
          fillBlanks: [{ id: 'b1', answer: 'return' }],
          hints: [
            { text: 'Funkcia má hotový výsledok v premennej result. Teraz ho musí poslať von.' },
            { text: 'V Pythone na vrátenie hodnoty z funkcie používame špeciálne kľúčové slovo.' },
            { text: 'Doplň:', code: 'return' },
          ],
          xp: 15,
        },
      ],
      ...
      steps: [
        {
          ...
          prompt: 'Kalkulačka musí vedieť, kedy má sčítať čísla. Ak používateľ vybral operáciu "+", čísla sa sčítajú. Doplň podmienku, ktorá porovná premennú operation s textom "+".',
          fillCode: 'if ___:\n    result = first_number + second_number',
          fillBlanks: [{ id: 'b1', answer: 'operation == "+"', alternatives: ['operation=="+"', "operation == '+'"] }],
          hints: [
            { text: 'Podmienka musí skontrolovať, či premenná operation obsahuje znak "+".' },
            { text: 'Na porovnanie dvoch hodnôt používame operátor ==' },
            { text: 'Doplň:', code: 'operation == "+"' },
          ],
        ...
        },
        {
          ...
          prompt: 'Prvá podmienka kontroluje sčítanie. Teraz pridáme druhú možnosť - odčítanie. Ak prvá podmienka (if) neplatí, Python skontroluje ďalšiu. Doplň kľúčové slovo, ktoré pridá ďalšiu podmienku za if.',
          fillCode: 'if operation == "+":\n    result = first_number + second_number\n___ operation == "-":\n    result = first_number - second_number',
          fillBlanks: [{ id: 'b1', answer: 'elif' }],
          hints: [
            { text: 'Kľúčové slovo pre "inak ak" - keď prvá podmienka neplatí, skontroluje sa ďalšia.' },
            { text: 'V Pythone sa to volá elif (skratka z else if).' },
            { text: 'Doplň:', code: 'elif' },
          ],
