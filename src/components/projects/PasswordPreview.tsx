...
const s = { bg: '#111', card: '#0c255a', border: '#0c255a', purple: '#8b5cf6', text: '#fff', dim: '#666' };
...
export default function PasswordPreview({ variables }: { variables: Record<string, any> }) {
  ...
  const getStrength = (pw: string) => {
    if (!pw) return { label: '', color: '#0c255a', width: 0 };
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {checks.map((c, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 13 }}>
            <div style={{
              ...
              background: c.active ? s.purple : 'transparent', border: `2px solid ${c.active ? s.purple : '#0c255a'}`,
              transition: 'all 0.2s',
            }}>
      ...
      {password && (
        <div style={{
          padding: '16px', background: '#000a2b', borderRadius: 12, border: '1px solid #0c255a',
          textAlign: 'center',
        }}>
          <div style={{
